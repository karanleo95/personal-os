...
<div style={{display:“flex”,gap:4,marginTop:6}}>{[1,2,3,4,5].map(i=><button key={i} onClick={()=>logEnergy(selDate,i)} style={{width:13,height:13,borderRadius:3,border:“none”,cursor:“pointer”,background:i<=dayEnergy?”#f59e0b”:C.subtle,padding:0}}/>)}</div>
</div>
</div>
</div>
...
{todo.map(h=>(<div key={h.id} style={s.row(false)}><button onClick={()=>onToggle(h.id)} style={{width:28,height:28,borderRadius:“50%”,border:`2px solid ${C.border}`,background:“transparent”,cursor:“pointer”,flexShrink:0,color:C.muted,fontSize:13,display:“flex”,alignItems:“center”,justifyContent:“center”}}>○</button><div style={{fontSize:20,flexShrink:0}}>{h.icon}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:C.text}}>{h.label}</div><div style={{fontSize:11,color:C.muted}}>{h.desc}</div></div></div>))}
{done.length>0&&<>
...
{[[“Start Weight”,“78 kg”],[“Target Weight”,“70 kg”],[“Waist Now”,“~88 cm”],[“Waist Target”,“~82 cm”],[“Body Fat Goal”,“Sub 18%”],[“Visceral Fat”,“10 → 7–8”]].map(([l,v],i,arr)=>(<div key={i} style={{padding:“12px 16px”,borderRight:(i+1)%2!==0?`1px solid ${C.border}`:“none”,borderBottom:i<arr.length-2?`1px solid ${C.border}`:“none”}}><div style={{fontSize:10,color:C.muted,marginBottom:2}}>{l}</div><div style={{fontSize:16,fontWeight:800}}>{v}</div></div>))}
</div>
</div>
{last8.length>=2&&<div style={s.card}><div style={s.ch(null)}><span style={{fontWeight:700,fontSize:14}}>📈 Weight Trend</span></div><div style={{padding:“14px 16px”,display:“flex”,alignItems:“flex-end”,gap:6,height:100}}>{last8.map((m,i)=>{const min=Math.min(…last8.map(x=>x.weight));const max=Math.max(…last8.map(x=>x.weight));const h=20+((m.weight-min)/(max-min||1))*60;const imp=i>0&&m.weight<=last8[i-1].weight;return(<div key={i} style={{flex:1,display:“flex”,flexDirection:“column”,alignItems:“center”,gap:3}}><div style={{fontSize:9,color:imp?”#10b981”:”#ef4444”,fontWeight:700}}>{m.weight}</div><div style={{width:“100%”,height:h,background:imp?”#10b98144”:”#ef444444”,borderRadius:“4px 4px 0 0”,border:`1px solid ${imp?"#10b981":"#ef4444"}`}}/><div style={{fontSize:8,color:C.muted}}>{m.date.slice(5)}</div></div>);}}</div></div>}
...
const weeks=Array.from({length:12},(*,wi)=>Array.from({length:7},(*,di)=>{const d=new Date(today);d.setDate(today.getDate()-((11-wi)*7)-(6-di));return d;}));
...
const Dot=({d,sz})=>{const day=habits[dateKey(d)]||{};const sc=HABITS.reduce((a,h)=>a+(day[h.id]?1:0),0);const p=Math.round((sc/HABITS.length)*100);const fut=d>today;const isT=dateKey(d)===todayKey();const bg=fut?”#1a1a18”:sc===0?”#252523”:scoreColor(p)+(p>=80?“ee”:p>=60?“99”:“55”);return <div title={`${d.toDateString()}: ${sc}/${HABITS.length}`} style={{width:sz,height:sz,borderRadius:2,background:bg,border:isT?“1.5px solid #f59e0b”:“1.5px solid transparent”}}/>;};
return <>
