...
const QUARTERS = [
  ...
  { id:"noon", label:"Noon", icon:"&#9728;", color:"#10b981", items:[
  ...
  { id:"night", label:"Night", icon:"🌙", color:"#6366f1", items:[
    {time:"8:30 PM",  icon:"[x]", task:"Log today's habits + calories in Healthify"},
...
function TodayTab({selDate,dayHabits,dayWater,dayEnergy,mealDay,habitScore,pct,quarter,setQuarter,isToday,isWed,toggleHabit,setWaterCount,logEnergy,quote,quoteLoad,setTab,s,C}) {
  return <>
    ...
    {isWed&&isToday&&<button onClick={()=>setTab("body")} style={{width:"100%",background:"#f59e0b11",border:"1px solid #f59e0b44",borderRadius:12,padding:"10px 16px",color:"#f59e0b",fontSize:13,fontWeight:700,cursor:"pointer",marginBottom:12,textAlign:"left"}}>[w] Wednesday - Log your weigh-in</button>}
...
function HabitCheckin({dayHabits,onToggle,s,C}) {
  ...
  return(
    <div style={s.card}>
      <div style={s.ch(null)}><span style={{fontWeight:700,fontSize:14}}>[x] Habit Check-in</span><span style={{fontSize:12,color:done.length===HABITS.length?"#10b981":C.muted,fontWeight:700}}>{done.length}/{HABITS.length}</span></div>
      ...
      {done.length>0&&<>
        ...
        {showDone&&done.map((h,i)=>(<div key={h.id} style={{...s.row(i===done.length-1),opacity:0.5}}><button onClick={()=>onToggle(h.id)} style={{width:28,height:28,borderRadius:"50%",border:"none",background:"#10b981",cursor:"pointer",flexShrink:0,color:"#fff",fontSize:13,display:"flex",alignItems:"center",justifyContent:"center"}}>v</button><div style={{fontSize:20,flexShrink:0}}>{h.icon}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:"#10b981",textDecoration:"line-through"}}>{h.label}</div></div></div>))}
      </>}
    </div>
  );
}
...
function MealsTab({s,C}) {
  ...
  return <>
    ...
    <div style={{...s.card,background:"#10b98111",border:"1px solid #10b98133"}}><div style={{padding:"12px 16px"}}><div style={{fontSize:11,color:"#10b981",fontWeight:700,marginBottom:4}}>🔒 BREAKFAST - FIXED - 9:30 AM</div><div style={{fontSize:13,color:"#10b981",lineHeight:1.8}}>Overnight oats 40g - Chia 1 tbsp - Flax 1 tbsp - Greek yogurt 100g - 5-6 strawberries - 4 walnuts - Apple or guava</div><div style={{fontSize:11,color:"#10b98188",marginTop:6}}>~420 kcal - 22g protein - 18g fibre</div></div></div>
    {days.map((day,i)=>{
      const d=weeks[week][i];const isSun=i===6;const isWed=i===2;
      return(<div key={i} style={{...s.card,opacity:isSun?0.65:1}}>
        <div style={{...s.ch(null),background:C.surface}}><span style={{fontWeight:700}}>{day}</span><div style={{display:"flex",gap:6}}>{isWed&&<span style={s.tag("#f59e0b")}>[w] Weigh-in</span>}{isSun&&<span style={s.tag("#ef4444")}>Flex</span>}</div></div>
...
function BodyTab({metrics,onAdd,s,C}) {
  ...
  return <>
    <div style={s.card}>
      <div style={s.ch(isWed?"#f59e0b":null)}><span style={{fontWeight:700,fontSize:14}}>{isWed?"[w] Wednesday Weigh-In":"📏 Log Metrics"}</span><span style={{fontSize:11,color:C.muted}}>{dateKey(new Date())}</span></div>
      <div style={{padding:"14px 16px"}}>
